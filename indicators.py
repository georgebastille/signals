...
def MACD(df, fastEMA=12, slowEMA=26, signal=9, base="Close"):
    ...
    # Compute MACD
    df[macd] = np.where(
        np.logical_and(np.logical_not(df[fE] == 0), np.logical_not(df[sE] == 0)),
        df[fE] - df[sE],
        0,
    )
    ...
    df[hist] = np.where(
        np.logical_and(np.logical_not(df[macd] == 0), np.logical_not(df[sig] == 0)),
        df[macd] - df[sig],
        0,
    )
...
def Ichimoku(df, ohlc=["Open", "High", "Low", "Close"], param=[9, 26, 52, 26]):
    ...
    df[senkou_span_a_column] = (
        (df[tenkan_sen_column] + df[kijun_sen_column]) / 2
    ).shift(kijun_sen_period)
    ...
    df[senkou_span_b_column] = ((senkou_span_high + senkou_span_low) / 2).shift(
        kijun_sen_period
    )
